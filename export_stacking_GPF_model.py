...
from config.svr_GPF_config import SVR_NAME 
from config.dnn_GPF_config import DNN_NAME, DEVICE
from config.stacking_GPF_config import *
from utils import get_best_svr_GPF_index, check_file_exists, clean_data
...
class Stacking:
    ...
    def __init__(self, final_estimator=Ridge()):
        ...
        best_svr_GPF_index = get_best_svr_GPF_index()
        self.model_filepaths = [
            os.path.join(
                MODEL_FOLDER, f"{SVR_NAME}_GPF",
                PARAMETERS_FOLDER, f"{best_svr_GPF_index}th_fold_model.pkl"
            ), # svr_GPF 
            os.path.join(
                MODEL_FOLDER, f"{DNN_NAME}_GPF",
...
def main():
    """Train Stacking GPF Model"""

    
    """ Set model name """
    stacking_name = f"{STACKING_NAME}_GPF"
